...
Содержмое поля traffic_direction, который необходим для создания
диаграммы:
...
Содержимое поля taffic_direction_in, который необходим для создания
диаграммы:
...
Прежде чем выполнять данное задание нужно определится с временными
рамками трафика. Выполним запрос в Yandex Quary и видим здесь, что
запросы выполняются меньше,чем 1 раз в секунду - каждые 10000 доли
секунд. Теперь нам нужно понять начальное и конечное время, когда по
сети проходил трафик:

![](./img/img9.png)

Выполним запрос в Yandex Quary и видим, что данные приходили примерно
1.5 секунды:
